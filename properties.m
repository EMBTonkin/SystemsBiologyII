function vv = prop_pub(Ca_in,ENa,EK,q,k,T,Ca_out,Cl_out,Cl_in,F,BC)
Far =F(9);
Kout = F(22);%
Naout = F(23);%
PCa=F(29);%
...
nPK=F(48);
% Permeability 
PK=(vPK*BC.^nPK)./(KPK+BC.^nPK);  %--> y(15,:)%PK=(.5+1*MPP.^-1./(5+MPP.^-1)); %==> Function of MP
...
c=-(4*PCa.*(Ca_out)*10^-3 + PK*(Kout) + PNa*(Naout)+PCl*(Cl_in)');
 ...
 vv= R*T/Far*2.303*log10(psi)*1000; %mVolt
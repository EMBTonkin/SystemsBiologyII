%%% This script reproduces the simulations used for Figures 3 and 4.
%%% Supplemental Figures 3 and 4 can also be reproduced in this script by
...
while w<=xaa  
    ...
    Shift_start=690; %%% How long after light entrainment to perform phase shift
    Phase_shift=12;  %%% How many hours to advance earlier the 24h circadian rhythm. Positive shifts shorten the intermediate periods.
...
colr=['x','+'];
color=nan(ncell,1);
pdnoGav=mean(phdiff(:,:,1:2:xaa*2-1),3);
pdwGav=mean(phdiff(:,:,2:2:xaa*2),3);
pdnoGstd=std(phdiff(:,:,1:2:xaa*2-1),[],3);
pdwGstd=std(phdiff(:,:,2:2:xaa*2),[],3);
kk=1:400;
...
colormap(jet);
cbarmax=colorbar('YTick',[1,length(jet)/2,length(jet)],'YTickLabels',{round(mn*100)/100,round((mn+mx)/2*100)/100,round(mx*100)/100},'FontSize',16);
...
colormap(jet);
cbarmax=colorbar('YTick',[1,length(jet)/2,length(jet)],'YTickLabels',{round(mn*100)/100,0,round(mx*100)/100},'FontSize',16);
...
jad(index,11:12)=[length(find(color==2)),ttest(mean(jack(find(color==2),2:2:iii)),mean(jack(find(color==2),1:2:iii-1)))];
